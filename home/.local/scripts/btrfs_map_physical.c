...
static int read_chunk_tree(int fd, struct chunk **chunks, size_t *num_chunks) {
  ...
  *num_chunks = 0;
  for (;;) {
    ...
    if (header->type != BTRFS_CHUNK_ITEM_KEY) goto next;
...
static int print_extents(int fd, struct chunk *chunks, size_t num_chunks) {
  ...
  puts(
      "FILE OFFSET\tFILE SIZE\tEXTENT OFFSET\tEXTENT TYPE\tLOGICAL "
  ...
  for (;;) {
    ...
    header = (struct btrfs_ioctl_search_header *)(search.buf + buf_off);
    if (header->type != BTRFS_EXTENT_DATA_KEY) goto next;
    ...
    if (item->encryption) printf(",encryption=%u", item->encryption);
...
out:
  for (i = 0; i < num_chunks; i++) free(chunks[i].stripes);
  free(chunks);
  close(fd);
