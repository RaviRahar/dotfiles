...
naughty.config.defaults.shape = function(cr, w, h)
    gears.shape.rounded_rect(cr, w, h, beautiful.xresources.apply_dpi(15))
end
...
elseif theme == "gruvbox_light" then
    ...
    beautiful.wallpaper = "/usr/share/backgrounds/wallpapers/7731818.png"
end
...
local mytagseparator = nil
local mynetspeedseparator = nil
if theme == "gruvbox_dark" then
    -- Dark Theme
    mytagseparator =
        wibox.widget.textbox('<span font="FiraCode Nerd Font 12" foreground="#458588" background="#222222"></span>')
    mynetspeedseparator =
        wibox.widget.textbox('<span font="FiraCode Nerd Font 12" foreground="#b16286" background="#222222"></span>')
elseif theme == "gruvbox_light" then
    -- Light Theme
    mytagseparator =
        wibox.widget.textbox('<span font="FiraCode Nerd Font 12" foreground="#458588" background="#ebdbb2"></span>')
    mynetspeedseparator =
        wibox.widget.textbox('<span font="FiraCode Nerd Font 12" foreground="#b16286" background="#ebdbb2"></span>')
end

local mynetworkseparator =
    wibox.widget.textbox('<span font="FiraCode Nerd Font 12" foreground="#83a598" background="#b16286"></span>')
local mybrightnesseparator =
    wibox.widget.textbox('<span font="FiraCode Nerd Font 12" foreground="#d3869b" background="#83a598"></span>')
local mysoundseparator =
    wibox.widget.textbox('<span font="FiraCode Nerd Font 12" foreground="#83a598" background="#d3869b"></span>')
local mybatteryseparator =
    wibox.widget.textbox('<span font="FiraCode Nerd Font 12" foreground="#fabd2f" background="#83a598"></span>')
local mydateseparator =
    wibox.widget.textbox('<span font="FiraCode Nerd Font 12" foreground="#282828" background="#fabd2f"></span>')
local mytimeseparator =
    wibox.widget.textbox('<span font="FiraCode Nerd Font 12" foreground="#ebdbb2" background="#282828"></span>')
local mymenuseparator =
    wibox.widget.textbox('<span font="FiraCode Nerd Font 12" foreground="#689d6a" background="#ebdbb2"></span>')
...
local mynetspeed = net_speed_widget({ width = 120 })
local mybattery = assault({
    battery = "BAT1",
    adapter = "ACAD",
    font = "NotoSans 14",
    width = 35,                 -- width of battery
    height = 20,                -- height of battery
    bolt_width = 20,            -- width of charging bolt
...
awful.screen.connect_for_each_screen(function(s)
    ...
    s.mypanel = awful.wibar({
        ...
        shape = function(cr, width, height)
            gears.shape.rounded_rect(cr, width, height, beautiful.xresources.apply_dpi(17))
        end
...
local rofi_theme_launcher = nil
local rofi_theme_dmenu = nil

if theme == "gruvbox_dark" then
    rofi_theme_launcher = "theme-launcher-gruvbox-dark-x"
    rofi_theme_dmenu = "theme-dmenu-gruvbox-dark"
elseif theme == "gruvbox_light" then
    rofi_theme_launcher = "theme-launcher-gruvbox-light-x"
    rofi_theme_dmenu = "theme-dmenu-gruvbox-light"
end
...
--awful.key({}, "XF86AudioMute", function() os.execute("pactl set-sink-mute @DEFAULT_SINK@ toggle") end),
    ...
    awful.key({ modkey, "Shift" }, "r", function()
        awful.spawn.with_shell(
            "rofi -show combi -c $HOME/.config/rofi/config.rasi -theme " .. rofi_theme_dmenu,
            awful.rules.rules
        )
    end, { description = "run prompt dmenu theme", group = "launcher" }),
    -- Menubar
    ...
    awful.key({ modkey }, "c", function()
        awful.spawn(terminal .. " --class nmtui -e nmtui-connect", awful.rules.rules)
    end, { description = "Launch NMTUI", group = "launcher" }),
    awful.key({ modkey }, "d", function()
        awful.spawn("thunar", awful.rules.rules)
    end, { description = "Launch Thunar", group = "launcher" }),
    awful.key({ modkey }, "y", function()
        awful.spawn("freetube")
    end, { description = "open Freetube", group = "launcher" }),
    awful.key({ modkey }, "n", function()
        awful.spawn("nvim", awful.rules.rules)
    end, { description = "Launch Neovim", group = "launcher" }),
    awful.key({ modkey }, "o", function()
        awful.spawn("emacsclient -nc", awful.rules.rules)
    end, { description = "Launch Emacs", group = "launcher" }),
    awful.key({ modkey, "Shift" }, "e", function()
        logout_popup.launch()
    end, { description = "Show logout screen", group = "launcher" }),
    awful.key({ modkey }, "s", function()
        awful.spawn("firefox-developer-edition", awful.rules.rules)
    ...
    end, { description = "focus the next screen", group = "awesome" }),
    ...
    end, { description = "cycle between active windows", group = "client" }),
...
local clientkeys = gears.table.join(
    ...
    awful.key({ modkey, "Shift" }, "q", function(c)
        awful.spawn("xkill", awful.rules.rules)
    end, { description = "close", group = "client" }),
...
local function move_mouse_to_client(c, skip_mouse_check)
    ...
    if
        mcc ~= nil -- disable for toolbars clicks
        and (
            skip_mouse_check
            or (
                not mouse.is_left_mouse_button_pressed
...
client.connect_signal("swapped", function(c1, c2) -- , is_source)
